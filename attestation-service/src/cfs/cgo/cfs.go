...
//export setResource
func setResource(path string, data string) *C.char {
	//
	fmt.Printf("confilesystem - go - setResource(): path = %v, data = %v\n", path, data)
...
//export getResource
func getResource(path string) *C.char {
	//
	fmt.Printf("confilesystem - go - getResource(): path = %v\n", path)

	resMap := make(map[string]interface{})
	resMap["ok"] = true
	resMap["data"] = "get-resource-return-data"
